...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cos = scalarMultiplication / (firstLength * secondLength);
...
function getFibonacciNumber(index) {
  ...
  for(let i = 2; i <= index; i++) {
    const sum = fibonacciArr.at(-1) + fibonacciArr.at(-2);
...
function getSumOfDigits(num) {
  return String(num).split('').reduce((acc, curr) => acc + +curr, 0);
}
